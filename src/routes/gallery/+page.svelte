...
<div class="container mx-auto px-4 py-8">
  ...
  {#if allMedia.length === 0}
    <div class="text-center py-16">
      <p class="text-muted-foreground text-lg">
        Zur Zeit gibt es noch keine Bilder. <br />Ab dem 18.04.2026 kannst du
  ...
  {:else}
    ...
    <Dialog bind:open={isOpen}>
      <DialogContent
        class="w-screen h-dvh max-w-none rounded-none border-0 p-0 bg-black overflow-hidden"
      >
        <Carousel
          opts={{ loop: true }}
        ...
        >
          <CarouselContent class="h-dvh">
            {#each allMedia as media, i}
              <CarouselItem
                class="h-dvh flex items-center justify-center overflow-hidden"
              >
                {#if media.type.startsWith("video/") && activeSlide === media.id}
                  <Video
                    {activeSlide}
                    id={media.id}
                    src={media.mediaUrl}
                ...
                {:else}
                  <Image
                    ...
                    class="max-w-full max-h-dvh object-contain select-none"
                    draggable="false"
                  />
                {/if}
              </CarouselItem>
